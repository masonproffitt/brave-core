...
export function MonthlyTipForm () {
  ...
  const [nextReconcileDate, setNextReconcileDate] = React.useState(host.state.nextReconcileDate)
  ...
  React.useEffect(() => {
    ...
    if (tipAmount === 0 && rewardsParameters && publisherInfo) {
      const options = generateTipOptions(rewardsParameters, publisherInfo)
      setTipAmount(getDefaultTip(options, currentMonthlyTip))
    }
  })
